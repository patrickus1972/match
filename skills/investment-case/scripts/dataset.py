...
class Dataset:
    ...
    def market(self, market_id: str) -> dict:
        for row in self.markets:
            if row["market_id"].upper() == (market_id or "").strip().upper():
                return row
        raise UnknownReference(f"'{market_id}' is not one of the four demo markets.")

    def mechanic(self, mechanic_id: str) -> dict:
        for row in self.mechanics:
            if row["mechanic_id"] == (mechanic_id or "").strip():
                return row
        raise UnknownReference(
            f"'{mechanic_id}' is not a known mechanic. Known mechanics: "
            + ", ".join(m["mechanic_id"] for m in self.mechanics))
...
def build_inputs(ds: Dataset, request: dict) -> calc.CaseInputs:
    ...
    return calc.CaseInputs(
        ...
        period_start_month=_opt_int(request.get("period_start_month", 1)),
        period_weeks=_opt_int(request.get("period_weeks", 52)),
        ap_request_eur=_opt_float(request.get("ap_request_eur")),
